...
### 第 3.5 步：Edge Case 分析（关键！）

找到根因后，**不要急着修代码**。先停下来做一次完整的 edge case 分析。

**目标：** 确保你的修改能覆盖 bug 描述中提到的**所有**参数组合和边界情况，而不是只修那一个崩溃点。

**怎么做：**

1. **列出相关函数的所有参数** -- 找到受影响方法的完整签名（包括所有参数和默认值）

2. **对每个参数，枚举所有可能的类型**：
   ```
   参数 secure 的可能取值：
   |-- None        -- 默认值，不启用安全连接
   |-- True        -- 启用但不验证（bug 描述提到的情况）
   |-- False       -- 明确禁用
   |-- ()          -- 空 tuple，历史兼容（starttls(*()) = starttls()）
   |-- []          -- 空 list（bug 描述提到的情况）
   |-- ('key','cert') -- 带证书的 tuple
   |-- {'keyfile':'x','certfile':'y'} -- dict 格式
   ```

3. **检查代码中这些参数的处理路径：**
   - 被直接 unpack 的地方 -> 会不会 TypeError？
   - 被 if 判断的地方 -> bool 值语义对不对？
   - 传给第三方库时 -> 格式是否兼容？

4. **特别关注「看似无关的路径」：**
   - 一个参数改了，其他参数的初始化逻辑也要同步调整（比如 `get_connection` 中 SMTP 连接、STARTTLS、credentials 认证是三个关联环节）
   - getter/setter、helper 函数、deprecation 警告
...
> :red_circle: **如果没有做完 edge case 分析就动手修，你修好的概率不到 50%。**

---

### 第 3.6 步：检查相关测试文件

如果仓库中有该模块的测试文件，仔细阅读它们——**这些测试反映了 API 的预期行为。**

```bash
# 找到相关测试文件
grep -rl 'MailHandler|get_connection|starttls' tests/ 2>/dev/null
```

读测试时关注：
1. **测试了哪些参数组合？** -- 这些是你必须支持的情况
2. **mock/patches 的使用模式？** -- 你的代码需要兼容 mock 的调用断言
3. **异常路径有测试吗？** -- 边界情况是否需要处理
...
1. **以 edge case 分析为指导** -- 你的修改必须覆盖第 3.5 步列出的所有参数类型组合，不只是 bug 描述中的那一个场景
2. **理解原作者的意图** -- 不要破坏正常逻辑。如果一个函数做了多件事（如 `get_connection` 处理连接、安全、认证），确保每一件事都正常工作
3. **保持 API 风格一致** -- 如果修改某个方法，检查同一个类中其他类似方法是怎么实现的。不要在同一个类中混用不同的编码风格
4. **一次改一个逻辑层** -- 修改参数处理时，追踪这个参数从初始化 -> 存储 -> 使用 的完整链路，不要只改使用的地方不改初始化
5. **如果多个位置都可能是根因，逐一排查，一次只试一个**
...
- ✅ 没有修改测试文件、配置文件、pytest 配置（模式 A）

---

### 第 6 步：diff Review（最终把关）

提交代码前，检查你的修改与代码库的整体一致性：
...
1. **改动量是否合理** -- 一个 bug fix 不应该改 10 个文件。如果改动量很大，可能方向不对
2. **代码风格是否一致** -- 看看你改的代码段，周围代码是怎么写的（异常处理方式、命名风格、传参风格），保持一致
3. **调用的 API 签名是否匹配** -- 如果你改了某个函数的参数处理方式，检查所有调用它的地方是否也需要同步修改
4. **Mock 兼容性** -- 如果你的修改改变了函数的调用约定（如从 `SMTP()` + `connect()` 改为 `SMTP(host, port)`），确认测试中的 mock 断言是否匹配

> :red_circle: 如果你的 diff 只改了 1-2 行（比如只加了一个 helper 函数），说明你可能只修了表面现象，没修到位。
...
- 不要删除测试或弱化断言来让测试通过
- 不要修改测试文件、注入 mock 或绕过外部服务（模式 B 中可新增测试文件，但不要删除或修改现有的）
- 模式 A 下不要修改源代码（模式 B 中按修复流程修改必要代码）
